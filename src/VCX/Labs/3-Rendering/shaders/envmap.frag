...
void main() {
    ...
    vec3 envSampleDir = reflect(-viewDir, normal); 
    envSampleDir = normalize(envSampleDir);
    vec3 envColor = texture(u_EnvironmentMap, envSampleDir).rgb;
    envColor = pow(envColor, vec3(gamma));
    vec3 environmentContribution = envColor * u_EnvironmentScale; 
    total += environmentContribution;
